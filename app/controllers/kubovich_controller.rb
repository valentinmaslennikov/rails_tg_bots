class KubovichController < Telegram::Bot::UpdatesController
  ...
  before_action :is_current_game?, only: [:participation, :slovo!, :task!, :drop_current_game!]
  before_action :is_active_player?, only: [:participation, :slovo!]
  ...
  def participation(*char)
    ...
    next_turn_message  = "#{next_turn_username} вращайте барабан/буква/слово целиком"

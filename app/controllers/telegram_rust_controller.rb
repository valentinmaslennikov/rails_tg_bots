...
class TelegramRustController < Telegram::Bot::UpdatesController
  ...
  def ban!(*args)
    if @user.username.eql?('loyalistscfa')
      User.find_by_username(args.first.to_s).update!(banned: true) rescue nil
    end
  end

  def unban!(*args)
    if @user.username.eql?('loyalistscfa')
      User.find_by_username(args.first.to_s).update!(banned: false) rescue nil
    end
  end
  ...
  def youtube_link(args = nil)
    ...
    res = JSON.parse(Faraday.get(CGI.escape(urlData)).body)

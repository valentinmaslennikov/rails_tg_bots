...
class TelegramRustController < Telegram::Bot::UpdatesController
  ...
  def message(message)
    ...
    if from['username']!='loyalistscfa' && message['text'] && message['text'].match(/(пид[оа]р|сука|сучара|бля[тд]?ь?|[хx]уй|[хx]у[ёе]|[хx]ул[ие]|пизд|за[её]ба|[её]ба[нт]|хуи[лщ][ае]|[у́́́́́е́])/i)
      bot.delete_message(chat_id: chat['id'], message_id: message['message_id'])
